		...
		if (loading) {
			...
			return (
				<div className={styles.container}>
					...
					<input className={styles.input} placeholder="0" min="0" type="number" onChange={(e) => setFakeNftTokenId(e.target.value)} />
				...
				<div>
					{proposals.map((p, index) => (
						<div key={index} className={styles.card}>
							<p>
								Proposal ID: <span className={styles.cardItemValue}>{p.proposalId}</span>
							</p>
							<p>
								Fake NFT to Purchase: <span className={styles.cardItemValue}>{p.nftTokenId}</span>
							</p>
							<p>
								Deadline: <span className={styles.cardItemValue}>{p.deadline.toLocaleString()}</span>
							</p>
							<p>
								Yay Votes: <span className={styles.cardItemValue}>{p.yayVotes}</span>
							</p>
							<p>
								Nay Votes: <span className={styles.cardItemValue}>{p.nayVotes}</span>
							</p>
							<p>
								Executed?: <span className={styles.cardItemValue}>{p.executed.toString()}</span>
							</p>
							{p.deadline.getTime() > Date.now() && !p.executed ? (
								...
								<div className={styles.flex}>
									<button className={(styles.button2, styles.buttonExecuteProposal)} onClick={() => executeProposal(p.proposalId)}>
		...
		<div className={inter.className}>
			...
			<div className={styles.main}>
				<div>
					...
					<div className={styles.flex}>
						<button className={(styles.button, styles.buttonCreateProposal)} onClick={() => setSelectedTab("Create Proposal")}>
							Create Proposal
						</button>
						<button className={(styles.button, styles.buttonViewProposal)} onClick={() => setSelectedTab("View Proposals")}>
							View Proposals
						</button>
					</div>
					{renderTabs()}
				...
				<div>
					<img className={styles.image} alt="LearnWeb3" src="https://i.imgur.com/buNhbF7.png" />
				</div>
			</div>
		</div>
	);
